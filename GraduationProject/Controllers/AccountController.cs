...
namespace GraduationProject.Controllers
{
    ...
    public class AccountController : ApiController
    {
        ...
        public async Task<IHttpActionResult> GetRole()
        {
            ...
            return Ok(new { id = id, roles = roles });
        }
        ...
        public async Task<IHttpActionResult> ForgotPassword(string userName)
        {
            ...
            if (user != null)
            {
                ...
                var body = "<p>Hello {0} <p>Wellcome to our web site </p> " +
                           "Please use <a href=\"{1}#/recover?token={2}&username={3}\">Link</a>" +
                           "<p>Thanks for using our services" +
                           "</p>";
                var host = HttpContext.Current.Request.UrlReferrer?.AbsoluteUri;
                await CommonService.SendMail(user.Email, "Reset Password", string.Format(body, user.UserName, host, code, user.UserName));
                return Ok();
            }
        ...
        // POST api/Account/RecetPassword
        [System.Web.Http.Route("RecetPassword")]
        [System.Web.Http.AllowAnonymous]
        [System.Web.Http.HttpPost]
        public async Task<IHttpActionResult> RecetPassword(ResetPasswordBindingModel model)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
