angular.module("module").controller("homeFundInvoice", ["$scope", "getAjax", "$http", "$location", "$routeParams",
            ...
            html2canvas(document.getElementById('parentReport'), {
                onrendered: function(canvas) {
                    ...
                    r.getDataUrl((s, d) => {
                        $http({
                            method: "POST",
                            url: "http://localhost:9408/api/Invoice?childId=" + $scope.childId + "&email=" + $location.search().mail,
                            data: { Value: s },
