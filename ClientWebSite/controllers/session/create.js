angular.module("module").controller("newSession", ["$scope", "getAjax", "$http", "$location", "postAjax", "auth", "serverUrl",
    function($scope, getAjax, $http, $location, postAjax, auth, serverUrl) {
        console.log('ssadsadasdsad');
        ...
        $scope.calc = (date) => {
            ...
            then(function(data) {
                ...
                $scope.content = from;
                $scope.slotIds = slots;
                getAjax.response(serverUrl + "api/Sessions/GetSessionByDate?date=" + date).
                then((response) => {
                    var ar = [];
